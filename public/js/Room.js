...
const swalBackground = 'rgba(255, 255, 255, 0.8)';
...
const _PEER = {
    ...
    startRec: '<i style="color: red;" class="fas fa-record-vinyl pulsate"></i>',
};
...
function whoAreYou() {
    ...
    Swal.fire({
        ...
        <div style="overflow: hidden;">
            <button id="initAudioButton" class="fas fa-microphone" onclick="handleAudio(event)" style="color:#376df9"></button>
            <button id="initVideoButton" class="fas fa-video" onclick="handleVideo(event)" style="color:#376df9"></button>
        </div>`,
...
function handleAudio(e) {
    ...
    setColor(e.target, isAudioAllowed ? '#376df9' : 'red');
    setColor(startAudioButton, isAudioAllowed ? '#376df9' : 'red');
}

function handleVideo(e) {
    isVideoAllowed = isVideoAllowed ? false : true;
    e.target.className = 'fas fa-video' + (isVideoAllowed ? '' : '-slash');
    setColor(e.target, isVideoAllowed ? '#376df9' : 'red');
    setColor(startVideoButton, isVideoAllowed ? '#376df9' : 'red');
}
...
async function shareRoom(useNavigator = false) {
    ...
    function share() {
        ...
        Swal.fire({
            ...
            <br/><br/>
            <p style="background:transparent; color:black;">Share this meeting invite others to join.</p>
            ...
            if (result.isConfirmed) {
                ...
                let message = {
                    email: '',
                    subject: 'Please join our TalkED Video Chat Meeting',
...
function handleButtons() {
    ...
    startRecButton.onclick = () => {
        rc.startRecording();
        rc.updatePeerInfo(peer_name, rc.peer_id, 'record', true);
    };
...
/*More Button - Side Bar Menu  ADDITIONAL*/
  
var a;
function open_close(){
  if(a==1){
    document.getElementById("mySidebar").style.width = "280px";
    document.getElementById("mySidebar").style.marginBottom = "250px";
    return a=0;}
  else if (isMobileDevice){
    document.getElementById("mySidebar").style.width = "280px";
    document.getElementById("mySidebar").style.height = "auto";
    document.getElementById("mySidebar").style.marginTop = "450px";
  }
  else if (isMobileDevice && a!=1){
